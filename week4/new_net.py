...
def simple_conv_net(input):
    ...
    dropout1 = fluid.layers.dropout(x=conv1, dropout_prob=0.2)
    conv2 = conv_block(dropout1, 64)
    dropout2 = fluid.layers.dropout(x=conv2, dropout_prob=0.3)
    conv3 = conv_block(dropout2, 128)
    fc1 = fluid.layers.fc(input=conv3, size=512, act='relu')
    bn = fluid.layers.batch_norm(input=fc1, act='relu')
    predict = fluid.layers.fc(input=bn, size=60, act='softmax')
    return predict
...
def train_program():
    ...
    accuracy = fluid.layers.accuracy(input=predict, label=label, k=5)
